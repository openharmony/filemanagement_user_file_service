...
#include "fileaccessserviceunregister_fuzzer.h"
...
bool DoSomethingUnregisterFuzzTest(sptr<IFileAccessServiceBase> proxy, const uint8_t* data, size_t size)
{
    if (data == nullptr) {
        return true;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::DoSomethingUnregisterFuzzTest(proxy, data, size);
    return 0;
}
}