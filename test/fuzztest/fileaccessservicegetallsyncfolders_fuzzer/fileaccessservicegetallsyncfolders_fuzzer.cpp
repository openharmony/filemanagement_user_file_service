...
#include "fileaccessservicegetallsyncfolders_fuzzer.h"
...
bool DoSomethingGetAllSyncFoldersFuzzTest(sptr<IFileAccessServiceBase> proxy, const uint8_t* data, size_t size)
{
    (void)data;
    (void)size;
    std::vector<SyncFolderExt> syncFolderExts;
    proxy->GetAllSyncFolders(syncFolderExts);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::DoSomethingGetAllSyncFoldersFuzzTest(proxy, data, size);
    return 0;
}
}