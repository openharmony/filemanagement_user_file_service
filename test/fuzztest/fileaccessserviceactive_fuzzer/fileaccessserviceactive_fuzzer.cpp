...
#include "fileaccessserviceactive_fuzzer.h"
...
bool DoSomethingActiveFuzzTest(sptr<IFileAccessServiceBase> proxy, const uint8_t* data, size_t size)
{
    if (data == nullptr) {
        return true;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::DoSomethingActiveFuzzTest(proxy, data, size);
    return 0;
}
}