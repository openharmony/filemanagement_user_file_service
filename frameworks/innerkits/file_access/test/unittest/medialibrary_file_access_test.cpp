...
HWTEST_F(FileAccessHelperTest, medialibrary_file_access_GetThumbnail_0000, testing::ext::TestSize.Level1)
{
    ...
    try {
        ...
        // size.width = 256;
        // size.height = 256;
        int result = g_fah->GetThumbnail(uri);
        // if (getPixelMap != nullptr) {
        //     auto res = getPixelMap->GetWidth();
        //     std::string resString = std::to_string(res);
        //     GTEST_LOG_(ERROR) << resString.c_str();
        //     GTEST_LOG_(ERROR) << "cjw test success";
        // }
        EXPECT_EQ(result, OHOS::FileAccessFwk::ERR_OK);
